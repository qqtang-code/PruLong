...
class MInference:
    ...
    def patch_model(self, model):
        ...
        if self.config.kv_type in ["snapkv", "pyramidkv"]:
            ...
            self.config.attn_kwargs.setdefault("compress_group_kvs", False)
            self.config.attn_kwargs.setdefault("n_rep", 4)

        if self.config.kv_type == "l2":
            self.config.attn_kwargs.setdefault("max_capacity_total", 4096)
            self.config.attn_kwargs.setdefault("num_skip_layers", 2)
