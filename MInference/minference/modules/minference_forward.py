...
def gather_last_q_vertical_slash_topk_v4_sparsity(self, q, k, v, head_id):
    # Let's do a simple approximation
    q_len = q.shape[2]
    k_len = k.shape[2]
    
    def dense_sparsity():
        return 0.0 
    def dilated_sparsity():
        return 1 - min(1.0, (1024 + 4096) / k_len)
    def stream_llm_sparsity(vertical_size, slash_size):
        return 1 - min(1.0, (vertical_size + slash_size) / k_len)
    def vertical_slash_sparsity(vertical_size, slash_size):
        vertical_size = min(q_len, max(vertical_size, 30))
        slash_size = min(q_len, max(slash_size, 50))
        return 1 - min(1.0, (vertical_size + slash_size) / k_len)
    def block_sparse_sparsity(blocks, block_size):
        block_num_q = (q_len - 1) // block_size + 1
        block_num_k = (k_len - 1) // block_size + 1
        return 1 - min(1.0, (blocks / (block_num_q * block_num_k)))
    
    if self.config.to_dict().get("dilated1", False) or self.config.to_dict().get("dilated2", False):
        return dilated_sparsity()
    if self.config.to_dict().get("dense", False):
        return dense_sparsity()
    if self.config.to_dict().get("a_shape", False) or self.config.to_dict().get("tri_shape", False):
        return vertical_slash_sparsity(self.config.streaming_kwargs["n_init"], self.config.streaming_kwargs["n_local"])
    if self.config.to_dict().get("static_pattern", False) or self.config.to_dict().get("vs_only", False):
        ty, vertical_size, slash_size, _ = self.best_pattern.get(head_id, ("vertical_and_slash", 1000, 6096, 1))
        return vertical_slash_sparsity(vertical_size, slash_size)
    if self.config.to_dict().get("stream_llm", False):
        return stream_llm_sparsity(self.config.streaming_kwargs["n_init"], self.config.streaming_kwargs["n_local"])
    if self.config.to_dict().get("block_sparse", False):
        return block_sparse_sparsity(100, 8)
    if self.config.to_dict().get("vertical_and_slash", False):
        ty, vertical_size, slash_size, _ = self.best_pattern.get(head_id, ("vertical_and_slash", 1000, 6096, 1))
        return vertical_slash_sparsity(vertical_size, slash_size)
