...
# Configuration
# ============================================================================
SCRIPT_NAME="localsize_sweep_pruduo"
DEFAULT_NUM_GPUS=1
...
OUT_DIR=outputs/${MODEL_NAME}_localsize/LOCAL${LOCALSIZE}_outputs${SUFFIX}__${TAG}
if [ -f ${OUT_DIR}/.${TASK_NAME}.completed ]; then
    echo "Skipping completed: ${MODEL_NAME} ${TAG} - ${TASK_NAME} - local${LOCALSIZE}"
    return
fi
...
JOB_NAME="${SCRIPT_NAME}_${TAG}_${TASK_NAME}_local${LOCALSIZE}_${MODEL_NAME}"
...
for TASK in ${COMMON_TASKS[@]}; do 
    for LOCALSIZE in 64 128 256 512 1024 2048; do
        # PruLong evaluation
        TAG="PRULONG"
        ...
        TASK=$TASK MODEL=$MODEL SUFFIX=$SUFFIX EXTRA=$EXTRA TAG=$TAG LOCALSIZE=$LOCALSIZE submit_job
