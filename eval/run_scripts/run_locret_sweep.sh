...
OUT_DIR=outputs/${MODEL_NAME}_locret/outputs${SUFFIX}__locret
...
#SBATCH --exclude=/home/tianyug/pli_node_k
#SBATCH --nodes=1
...
PREFILL=32768
LOCRET_BIN_FILE=<your_path>/llama-3.1-8B-instruct.bin
LOCAL_LEN=100
STABILIZERS=2500
...
for TASK in ${COMMON_TASKS[@]}; do 
    for SPARSITY in 20 30 40 50 60 70 80 90; do
        ...
        EXTRA="--no_torch_compile --locret_bin_file $LOCRET_BIN_FILE --locret_sparsity $SPARSITY_FRAC --locret_local_len $LOCAL_LEN --locret_stabilizers $STABILIZERS"

        # Set suffix based on prefill settings
        if [[ $PREFILL -gt 0 ]]; then
            SUFFIX="_sp${SPARSITY}_pf${PREFILL}_tg"
            EXTRA="$EXTRA --locret_chunk_prefilling $PREFILL"
        else
            SUFFIX="_sp${SPARSITY}_tg"
        fi
    
        TASK=$TASK MODEL=$MODEL SUFFIX=$SUFFIX EXTRA=$EXTRA SPARSITY=$SPARSITY submit_job
    done
done
