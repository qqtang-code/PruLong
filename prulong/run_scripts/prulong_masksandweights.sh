#!/bin/bash
#SBATCH --job-name=prulong_masksandweights
...
# Continued pretraining arguments
max_toks=${MAX_TOKS:-131072}
start_head_sparsity=${START_HEAD_SPARSITY:-0.7}
...
# Mask loading configuration
load_masks_from=${LOAD_MASKS_FROM:-"${model}/masks_sp0.7.tsv"}
load_masks_sparsity=${LOAD_MASKS_SPARSITY:-$start_head_sparsity}
...
dataset=${DATASET:-"datasets/prolong-sample"}
...
run_name="masksandweights_$(basename $model)_bsz${bsz}_steps${steps}_lr${lr}_warmup${warmup}_sp${end_head_sparsity}_cw${context_window_if_toggled}_mlr${mask_learning_rate}_rlr${reg_learning_rate}${suffix}${extra_name}_${timestamp}"
...
base_arguments=(
    ...
    --load_masks_from $load_masks_from
    --load_masks_sparsity $load_masks_sparsity
